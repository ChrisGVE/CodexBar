...
struct MenuCardModelTests {
    ...
    @Test
    func `codex primary metric exposes session pace after minimum elapsed`() throws {
        // 50% elapsed in a 5h session, 50% used → on pace.
        ...
        let primary = try #require(model.metrics.first)
        #expect(primary.detailLeftText == "On pace")
    }

    @Test
    func `codex primary metric suppresses session pace before minimum elapsed`() throws {
        // Only ~10 min into a 5h session — well below the 10% guard.
        let now = Date(timeIntervalSince1970: 0)
        let identity = ProviderIdentitySnapshot(
            providerID: .codex,
            accountEmail: "codex@example.com",
        ...
        let snapshot = UsageSnapshot(
            primary: RateWindow(
                usedPercent: 5,
                windowMinutes: 300,
                resetsAt: now.addingTimeInterval(290 * 60),
                resetDescription: nil),
            secondary: nil,
            tertiary: nil,
            updatedAt: now,
    ...
    @Test
    func `non session provider primary metric has no session pace`() throws {
        let now = Date(timeIntervalSince1970: 0)
        let identity = ProviderIdentitySnapshot(
            providerID: .zai,
            accountEmail: nil,
