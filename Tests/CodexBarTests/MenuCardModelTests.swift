...
struct MenuCardModelTests {
    ...
    @Test
    func `metric exposes burn rate text when provided`() throws {
        let now = Date()
        let identity = ProviderIdentitySnapshot(
            providerID: .codex,
            accountEmail: "codex@example.com",
        ...
        let model = UsageMenuCardView.Model.make(.init(
            ...
            hidePersonalInfo: false,
            now: now,
            primaryBurnRateText: "12.4 %/h",
            secondaryBurnRateText: "1.1 %/h"))

        let primary = try #require(model.metrics.first { $0.id == "primary" })
        #expect(primary.burnRateText == "12.4 %/h")
        let secondary = try #require(model.metrics.first { $0.id == "secondary" })
        #expect(secondary.burnRateText == "1.1 %/h")
    }
